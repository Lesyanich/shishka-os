 ...
 */
export function CashReceiptPage({ data }: { data: PayslipData }) {
...
/** One receipt, one file — what the Payslip modal downloads. */
export function CashReceiptPdf({ data }: { data: PayslipData }) {
  return (
    <Document>
      <CashReceiptPage data={data} />
    </Document>
  )
}

/**
 * Every receipt for one wage period in a single file, one employee per page.
 *
 * Still one sheet per employee once printed, so the salary-privacy decision
 * holds — this only saves handling seven separate downloads on payday.
 */
export function CashReceiptBatchPdf({ rows }: { rows: PayslipData[] }) {
  return (
    <Document>
      {rows.map((data) => (
        <CashReceiptPage key={data.line.id} data={data} />
      ))}
    </Document>
  )
}
