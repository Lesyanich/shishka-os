...
export function useDishRecipeSteps(
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data, error: err } = await supabase
      .from('recipes_flow')
      .select(
        'id, step_order, operation_name, duration_min, instruction_text, temperature_c, internal_temp_c, equipment(name, category), notes, is_ccp, ccp_check_text, is_passive, location_id, location:locations(name, type)',
      )
