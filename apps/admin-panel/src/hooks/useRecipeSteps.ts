...
export function useRecipeSteps(): UseRecipeStepsResult {
  ...
  const mapRows = (data: Record<string, unknown>[]): RecipeStep[] =>
    data.map((row) => ({
      ...
      location_name: (row.location as { name: string; type: string } | null)?.name ?? null,
      location_type: (row.location as { name: string; type: string } | null)?.type ?? null,
    }))

  const SELECT_COLS = 'id, step_order, operation_name, instruction_text, duration_min, equipment_id, equipment(name), temperature_c, internal_temp_c, is_passive, notes, location_id, location:locations(name, type)'
