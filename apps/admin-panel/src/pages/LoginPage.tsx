...
import { staffNameToEmail } from '../lib/staffAuth'
...
export function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const creds =
      mode === 'staff'
        ? { email: staffNameToEmail(name), password: pin }
        : { email, password }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === 'staff' ? (
            <>
              ...
              <div>
                <label htmlFor="pin" className="mb-1 block text-xs font-medium text-slate-400">
                  PIN
                </label>
                <input
                  id="pin"
