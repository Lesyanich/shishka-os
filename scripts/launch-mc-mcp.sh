#!/usr/bin/env bash
# launch-mc-mcp.sh — starts the shishka-mission-control MCP server.
...
ENTRY="$REPO_ROOT/services/mcp-mission-control/dist/index.js"
if [ ! -f "$ENTRY" ]; then
  echo "[launcher] ERROR: $ENTRY not found. Build the service:" >&2
  echo "  cd $REPO_ROOT/services/mcp-mission-control && npm install && npm run build" >&2
  exit 1
fi
