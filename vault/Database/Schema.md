...
| `purchase_orders` | `id` UUID | po_number (UNIQUE), status (po_status), expected_date, delivery_window, subtotal, discount_total, vat_amount, delivery_fee, grand_total, created_by | supplier_id -> suppliers, deliver_to_station_id -> stations, source_plan_id -> production_plans, expense_id -> expense_ledger | 061, 379 |
| `po_lines` | `id` UUID | qty_ordered (CHECK > 0, mig 324), unit, unit_price_expected, total_expected (GENERATED), sort_order, UNIQUE(po_id, nomenclature_id, COALESCE(sku_id,'000…0')) — **one product per order**, so an "add item" flow must exclude lines already present; trg_po_lines_rollup rolls totals up to purchase_orders | po_id -> purchase_orders (CASCADE), nomenclature_id -> nomenclature, sku_id -> sku, destination_station_id -> stations | 061, 323, 324, 379 |
...
| `tag_group` | dietary, allergen, functional, storage, quality, cuisine, technique, taste, boosters, science, serving, ops | tags.tag_group |
...
## DB Normalization & Seed Data (Phase 7.1, Migration 067)

### Changes
- **tag_group ENUM**: 5 new values: `taste`, `boosters`, `science`, `serving`, `ops`
- **tags**: 21 new tags across new groups. `name_th` backfilled for all 37 existing tags. `keto` → `keto-lowcarb` merge; `low-carb` deprecated.
- **product_categories**: New `F-ZW` (L2) + `F-ZW-BYP` (L3, default_fin_sub_code=4101) for Zero-Waste By-products. All L3 nodes now have NOT NULL `default_fin_sub_code` (CHECK constraint enforced).
- **fin_sub_categories**: 4 new codes — 2303 (Cleaning Supplies), 2304 (Office Supplies), 4150 (Internal Production), 4203 (Disposables).
