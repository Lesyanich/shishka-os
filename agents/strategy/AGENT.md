...
2. **Read MC state (strategic lens):**
   ```
...
3. **Compute push triggers (strategic, max 3):**
...
4. **Report to CEO** in this exact shape (Russian, per spec §2.1):
   ```
...
|---|---|---|
| "What did we decide last time about X?" | Native auto-memory files | Read auto-memory |
| "What is <bible-id>?" / "What's our SOP?" | Project docs | Read `docs/bible/*` or `docs/business/*` |
...
- **Tier 2 (native auto-memory):** session reflection, classification reasoning, draft text
