-- ═══════════════════════════════════════════════════════════════════
-- Migration 041: delivery_fee Column + fn_approve_receipt v7
-- Phase 6.6 — Syrve-Grade Invoicing & Data Sanitization
-- ═══════════════════════════════════════════════════════════════════
-- Problem: Delivery fees on receipts (e.g. Makro delivery 40 THB)
-- are not captured, causing reconciliation mismatches.
--
-- Fix: Add delivery_fee column to expense_ledger (Hub).
-- Update fn_approve_receipt to v7: passes delivery_fee to Hub INSERT.
-- delivery_fee is a positive number, stored at Hub level only
-- (NOT distributed to Spokes — it's a receipt-level modifier).
-- ═══════════════════════════════════════════════════════════════════

-- ── 1. Add delivery_fee column to expense_ledger ──
ALTER TABLE expense_ledger
  ADD COLUMN IF NOT EXISTS delivery_fee NUMERIC DEFAULT 0;

COMMENT ON COLUMN expense_ledger.delivery_fee
IS 'Phase 6.6: Delivery/shipping charge (positive number). 0 if no delivery.';

-- ── 2. fn_approve_receipt v7: pass delivery_fee to Hub ──
...
IS 'Phase 6.6: v7 — delivery_fee in Hub INSERT. v6 UoM conversion preserved. Hub+3Spokes.';
