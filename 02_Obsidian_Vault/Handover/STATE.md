...
## Phase 4.16: Smart Tiling — Width-Preserving Receipt Compression (2026-03-11)

**Problem:** Phase 4.15's zero-footprint approach passes 15MB raw public URLs to OpenAI, but OpenAI's crawlers timeout downloading large files (`HTTP 400: Timeout while downloading... invalid_image_url`). Can't revert to old `compressImage()` either — it crushes tall receipt width to ~256px via `ratio = min(2048/w, 2048/h)`.
