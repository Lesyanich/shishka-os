-- Migration 210: Auto-assign category_id on nomenclature INSERT
-- MC task b1b7d18c
--
-- Fires BEFORE INSERT when category_id IS NULL.
-- Pattern-matches product name (case-insensitive) against keyword→category map.
-- Also checks supplier_catalog for existing category hints.
-- Fallback: NULL (human review via data_health).
...
BEGIN
  ...
  -- ── 1. Try supplier_catalog hint first ──
  SELECT sc_cat.category_id INTO v_cat_id
  FROM supplier_catalog sc_cat
  WHERE sc_cat.nomenclature_id = NEW.id
    AND sc_cat.category_id IS NOT NULL
  LIMIT 1;
  ...
  -- ── 2. Keyword pattern matching on name ──
  v_cat_code := CASE
    -- Dairy
...
COMMENT ON FUNCTION public.fn_auto_assign_category() IS
  'BEFORE INSERT trigger on nomenclature: auto-assigns category_id by keyword matching product name to L3 product_categories. Only fires for RAW-* items with NULL category_id. Checks supplier_catalog first, then keyword map. Falls back to NULL for human review.';

-- Drop if exists (idempotent)
DROP TRIGGER IF EXISTS trg_nomenclature_auto_category ON public.nomenclature;

CREATE TRIGGER trg_nomenclature_auto_category
  BEFORE INSERT ON public.nomenclature
  FOR EACH ROW
  EXECUTE FUNCTION public.fn_auto_assign_category();

INSERT INTO migration_log (filename, applied_by, notes)
VALUES (
    '210_nomenclature_auto_category_trigger.sql',
    'claude-code',
    'Auto-assign category_id on nomenclature INSERT via keyword pattern matching. RAW-* only, NULL fallback. MC b1b7d18c.'
