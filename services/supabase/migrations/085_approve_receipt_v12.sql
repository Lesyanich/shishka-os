-- ============================================================
-- Migration 085: fn_approve_receipt v12
-- supplier_catalog learning loop + purchase_logs.barcode + capex asset_id
-- ============================================================
-- CHANGES FROM v11:
--   1. ADD barcode column to purchase_logs
--   2. supplier_catalog learning loop: after each food_item,
--      UPSERT supplier_catalog to track mappings and prices
--   3. Save barcode into purchase_logs for direct traceability
--   4. Accept optional asset_id in capex_items for capex_assets linkage
--   5. Save raw_parse directly in Hub INSERT (no separate UPDATE needed)
--   6. Return supplier_catalog_updated count
--
-- All v11 logic preserved: receiving_records, receiving_lines,
-- SKU resolution, sku_balances UPSERT, Hub+3Spokes.
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_pl_barcode
  ON public.purchase_logs(barcode) WHERE barcode IS NOT NULL;
...
BEGIN
  ...
  -- ── 7. Return success — v12: includes supplier_catalog_updated ──
...
COMMENT ON FUNCTION public.fn_approve_receipt(JSONB)
  IS 'v12 — Adds supplier_catalog learning loop (UPSERT on each food item), purchase_logs.barcode, SKU barcode backfill, capex asset_id passthrough, raw_parse in Hub INSERT. All v11 logic preserved.';
