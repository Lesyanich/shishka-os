-- ============================================================
-- Migration 176: fn_approve_receipt v15
-- Fix: unmatched food_items no longer silently CONTINUE.
-- Instead: create RAW-AUTO nomenclature item + always write
-- purchase_logs so WAC trigger fires. Backfill supplier_catalog
-- with the new nomenclature_id.
--
-- Root cause (v14): line "CONTINUE; -- skip purchase_logs/sku"
-- after unmatched_items insert meant 76% of Makro receipt items
-- never reached purchase_logs → WAC stayed 0 → dish costing wrong.
--
-- Change: replace CONTINUE with RAW-AUTO creation, then fall
-- through to normal purchase_logs/sku/receiving_lines flow.
-- All existing matching cascade (barcode→SKU→fuzzy) untouched.
-- ============================================================
...
BEGIN
  ...
  IF p_payload->'food_items' IS NOT NULL
    ...
    LOOP
      ...
      -- ── v15 CHANGE: RAW-AUTO fallback instead of CONTINUE ──
      IF v_nom_id IS NULL THEN
        -- Log to unmatched_items for review
        ...
        -- Create supplier_catalog entry for barcodes (same as v14)
        ...
        -- ╔══════════════════════════════════════════════════════╗
        -- ║  v15 NEW: Create RAW-AUTO item instead of CONTINUE  ║
        -- ║  This ensures purchase_logs always gets a row        ║
        -- ║  and WAC trigger fires for every receipt item.       ║
        -- ╚══════════════════════════════════════════════════════╝
        v_auto_product_code := 'RAW-AUTO-' || substr(md5(v_item_name || COALESCE(v_item_barcode, '')), 1, 8);

        INSERT INTO public.nomenclature (product_code, name, type, base_unit, is_available)
        VALUES (v_auto_product_code, v_item_name, 'raw_ingredient', COALESCE(v_item_unit, 'pcs'), true)
        ON CONFLICT (product_code) DO UPDATE SET updated_at = now()
        RETURNING id INTO v_nom_id;

        v_auto_count := v_auto_count + 1;

        -- Backfill supplier_catalog with the new nomenclature_id
        ...
        -- Mark unmatched_items as auto-resolved
        UPDATE public.unmatched_items
        SET resolved_to = v_nom_id,
            resolved_at = now()
        WHERE expense_id = v_expense_id
          AND raw_text = v_item_name
      ...
      IF v_supplier_id IS NOT NULL THEN
        -- Only run catalog upsert for MATCHED items (not RAW-AUTO)
        -- RAW-AUTO items already handled above
        IF v_auto_product_code IS NULL THEN
          v_sc_id := NULL;
      ...
      v_auto_product_code := NULL;  -- reset for next iteration
    END LOOP;
  END IF;
...
COMMENT ON FUNCTION public.fn_approve_receipt(JSONB)
  IS 'v15 — Unmatched food_items now create RAW-AUTO nomenclature items and always write purchase_logs (triggering WAC update). No more silent CONTINUE. All v14 matching cascade preserved.';

-- ── Self-register in migration_log ──
INSERT INTO public.migration_log (filename, applied_by, description)
VALUES (
  '176_approve_receipt_v15_always_purchase_logs.sql',
  'claude-code',
  'fn_approve_receipt v15: unmatched items create RAW-AUTO nomenclature + always write purchase_logs. Fixes 76% item loss (294/387 food_items never reached purchase_logs in v14).'
