...
//   POST ?action=resync_names           → rename every synced SALE dish in place to its
//                                          staff-code-prefixed name, preserving modifiers
//
...
// ── Staff-code naming (mig 221/222) ──
// POS item name carries the staff code as a prefix, e.g. "S-1 Mango Smoothie".
...
async function handleRecreateItem(req: Request) {
  ...
  try {
    ...
    await db.from("nomenclature").update({ loyverse_item_id: newId, pos_status: "synced", loyverse_synced_at: new Date().toISOString() }).eq("id", body.dish_id)
    return json({
      ok: true,
...
async function handlePushDish(dishId: string) {
  ...
  try {
    ...
    if (loyverseItemId) {
      await db.from("nomenclature").update({ loyverse_item_id: loyverseItemId, pos_status: "synced", loyverse_synced_at: new Date().toISOString() }).eq("id", dishId)
    }
...
// ── Action: resync_names ──
// Rename every synced, staff-coded SALE dish in place to its prefixed name
// ("S-1 …"), preserving variants and re-binding modifiers from
// dish_modifier_groups (our source of truth). Idempotent: skips items whose
// Loyverse name already matches. Stable Loyverse item id (no delete/recreate).
        ...
        return json(
          {
            ok: false,
            error:
              "Unknown action. Use: status, categories, push_dish, pull_modifiers, create_modifier, get_item, recreate_item, update_item, resync_names, add_modifier_option, remove_modifier_option, ensure_modifier_stores, item_modifiers, push_modifiers",
          },
          400,
        )
    }
