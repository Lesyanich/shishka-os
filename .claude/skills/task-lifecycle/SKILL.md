...
Your unique session ID is in `.claude/.session-id` — auto-generated by `scripts/session-start.sh` on session start. The SessionStart hook also injects a live snapshot of `in_progress` MC tasks into your context, marking any owned by other sessions with `⚠ OWNED BY ANOTHER SESSION`.

```sh
MY_SESSION_ID=$(cat .claude/.session-id)
```

If `.claude/.session-id` is missing for any reason, regenerate via:
```sh
sh scripts/new-session-id.sh > .claude/.session-id
```
...
- Else → **HARD CLAIM-GATE RULE** (enforced by `.claude/hooks/claim-gate-pretool.sh`):
  - Tasks with `claimed_by != MY_SESSION_ID` and `claimed_at < 2h ago` are **OFF-LIMITS**.
  - Do NOT show them as candidates. Do NOT call `update_task(status='in_progress')` on them — the PreToolUse hook will deny the call with `permissionDecision: "deny"`.
  - If you must surface such a task to the CEO, format: `занято by {claimed_by}, branch {git_branch}, since {claimed_at}`.
- **Stale check** (only if `claimed_at` > 2h ago):
  - AND no git commits on `related_ids.git_branch` in the last **90 minutes** (check with `git log --since="90 minutes ago" origin/{branch}`)
  - If BOTH true → offer CEO takeover: "Task [title] was claimed by [session] at [time] but shows no git activity for 90+ min. Take over?"
  - **NEVER auto-takeover.** Always wait for CEO approval. The hook allows stale takeovers (>2h) but you still must ask the CEO first.
...
Session ID format: `claude-{model}-session-{suffix}` where `suffix` is either the first 8 chars of Claude Code's own `session_id` (preferred — globally unique) or a fallback `MMDD-HHMM-rand4` (e.g. `claude-opus-session-abc12345` or `claude-opus-session-0428-1050-700d`). The legacy `MMDD`-only format is **deprecated** — it caused same-day collisions (incident 2026-04-28, task 2d709466). Always read your ID from `.claude/.session-id` rather than constructing it inline.
