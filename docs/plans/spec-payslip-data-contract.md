...
| 1c | ~~Public holidays are invisible~~ — **CORRECTED 2026-07-31** | The original claim, "no holidays/calendar table exists anywhere in the schema", was **wrong**. `public_holidays` has existed since migration 170a with 17 seeded rows for 2026, and `useScheduleTemplates` already skips those dates when generating shifts — which is exactly why 28 July held zero shifts while every other July weekday held 3-4. The real gaps were narrower: no way to record that we *traded through* a holiday, no ledger of what that earns, and two lunar holidays (Asalha Bucha, Khao Phansa, both in July) missing from the seed. Fixed in migration 397. | Medium — narrower than first assessed |
...
**Corrected.** A holiday reference DOES exist — `public_holidays`, migration 170a, 17 rows seeded for 2026, already consumed by `useScheduleTemplates` (skips holidays when generating shifts) and `use-attendance.ts`. What was missing, and what migration 397 added:
