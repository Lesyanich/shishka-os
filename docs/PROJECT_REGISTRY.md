...
> **Правило репозитория:** кто **пишет в базу** — тот **shishka-os**. Кто **только показывает
> гостю** — тот **shishka-health**. Сомневаешься → `shishka-os`. Новое поле всегда рождается
> в `shishka-os` (миграция) и только потом появляется в `shishka-health` (отрисовка).
...
| Меню, вёрстка, тексты, дизайн-система публичного сайта | **Menu Website** (репо `shishka-health`) | Claude Code |
| Приём заказов / доставка (когда начнем) | **Delivery Web** (`apps/web/`, не начат) | Claude Code |
...
## 6. Menu Website (живой)

**ID:** `menu-web`
**Инструмент:** Claude Code (терминал)
**Статус:** Live — https://shishka.health
**Репозиторий:** `Lesyanich/shishka-health` → `~/code/shishka-health` (**отдельный репо, не папка внутри shishka-os**)
**Деплой:** Vercel-проект `shishka-web`, только через git. Подробности и разбор аварии: `docs/operations/deploy-map.md`

### Назначение
Публичное меню для гостя: витрина, фильтры, конструктор наборов. **Только читает базу** —
ни одной записи. Здесь же живёт бренд-дизайн-система (`design-system/index.html` + `MASTER.md`),
которая является источником правды для бренда — см. `technical-rules.md` § RULE-DESIGN-SYSTEM.

### Связь с базой
Читает `menu_public`, `site_content`, `menu_modifiers`, `price_tiers` анонимным ключом.
Это **контракт**: любая миграция, трогающая эти объекты, ломает живой сайт.
Список колонок — `contracts/menu-contract.json` в репо сайта.

### Когда открывать
Внешний вид, вёрстка, тексты-дефолты, дизайн-система. Данные меню (цены, состав, фото)
меняются не здесь, а в админке или через `/chef`.

---

## 7. Delivery Website (будущий)

**ID:** `delivery-web`
**Инструмент:** Claude Code (терминал)
**Статус:** Planned — не начат
**Папка:** `apps/web/` внутри `shishka-os`

### Назначение
Приём заказов и доставка. **Это не сайт меню** — меню уже живёт в репо `shishka-health` (§6).
Когда начнём, потребуется отдельный Vercel-проект, никогда не `shishka-web`.
