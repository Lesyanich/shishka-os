...
- Migration 074: recipes_flow v2 — `nomenclature_id` UUID FK (was product_code TEXT), новые поля: `temperature_c`, `internal_temp_c`, `is_passive`, `duration_min` (was expected_duration_min)
- Migration 075: production_orders — новая таблица для производственных заданий
- Migration 076: equipment_slots enrichment + seed capacity data для ключевого оборудования
- manage_recipe_flow: переписан на `nomenclature_id` (вместо product_code FK), добавлены поля temperature_c, internal_temp_c, is_passive. Ответ list теперь включает active_duration_min / passive_duration_min
- remove_bom_line: исправлен `quantity` → `quantity_per_unit` (соответствие реальной схеме bom_structures)
...
## Schema Mapping (MCP ↔ DB)
MCP-агент использует человеко-понятные имена инструментов, но все запросы идут в реальные таблицы:

| MCP tool says | Actual DB table | Notes |
|---|---|---|
| "products" | `nomenclature` | search_products, create_product, update_product |
| "bom" | `bom_structures` | add_bom_line, remove_bom_line, get_bom_tree |
| "recipe flow" | `recipes_flow` | manage_recipe_flow (FK: nomenclature_id UUID) |
| "equipment" | `equipment` | list_equipment (enriched: capacity, setup_time_min) |
| "inventory" | `v_inventory_by_nomenclature` | check_inventory (VIEW over sku_balances) |
