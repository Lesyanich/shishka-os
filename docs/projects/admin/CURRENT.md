...
- **2026-06-30 — Browser-free Loyverse push via DB queue + pg_cron (branch `feature/menu/loyverse-push-queue`).**
  ...
  - **NOT yet activated** — one-time CEO setup required, order-sensitive (secret + deploy + vault BEFORE migration, else drain 401-loops). Runbook: [`docs/handoffs/loyverse-push-queue-activation.md`](../../handoffs/loyverse-push-queue-activation.md).
  - Phase 1 scope = `dish`/`prices`/`names` only (low risk). **Modifiers gated** behind a one-time mirror reconcile (drift since 2026-06-15; global reattach can corrupt POS attachments) — Phase 3. Admin queue-status panel = Phase 2 follow-up. NB: admin ALREADY binds modifier groups↔dishes via `useDishModifierGroups` (the owner's suspected gap doesn't exist). 0 `approved` dishes currently, so first push test uses a `synced` dish.
