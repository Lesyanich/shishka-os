...
- **2026-07-08 — Repo-root & CLAUDE.md hygiene (branch `feature/cleanup/ai-config-hygiene`, MC 07044b76, epic 2a8b06a4).**
  ...
  - Verified NOT junk, left alone: `graphify-out/` (manifest whitelisted, used by settings.json hook + graphify skill). **Superseded 2026-07-18:** graphify was retired entirely — the directory, the hook and the skill are all gone. See `docs/plans/spec-graphify-retirement.md`.
...
- **2026-07-03 — Chef agent recalibration: culinary-science foundation + brand red-line enforcement (branch `feature/agents/chef-recalibration`).**
  ...
  - **Loading + gate:** rewrote `.claude/commands/chef.md` to load kitchen-philosophy + the foundation + mandatory `recall_memories` before greeting; `AGENT.md` gets RULE-GROUNDING-GATE (red line → real spec+true cost → name mechanism/WebSearch → ESTIMATE → process check) wired into WF-1/3/7 + Rules 17–20. 4 corrections added to `chef-preferences.md` and stored to `agent_memory` (type=correction) so they resurface every session.
  - **Brand red lines:** `kitchen-philosophy.md` §2 now names rice-bran + all RBD seed/grain oils banned, gates animal fats, adds Approved Fats Matrix (deodorized-coconut exception) + Protocol 4. **Code enforcement** in `services/mcp-chef` — new `lib/brand-rules.ts` (BANNED/GATED patterns, nut-butter false-positive guard); `create_product` hard-errors banned / confirm-gates animal fats; `add_bom_line` blocks banned children; `validate_bom`+`audit_all_dishes` sweep for `RED_LINE_VIOLATION` in existing BOMs. `tsc` green, 10/10 brand-rules unit tests pass.
  - **SOPs:** committed `sop-shrimp-prep.md` into VCS (was uncommitted in main checkout) + new `sop-salmon-prep.md`. ⚠️ **Flagged conflict:** the shrimp marinade + live `PF-MARINADE_GARLIC_HERB` recipe use **rice-bran oil** — now banned. Not silently changed; MC task opened to reformulate → avocado oil (pending CEO sign-off). Eval doc: `agents/chef/evals/recalibration-2026-07.md`.
...
- **2026-07-01 — Prep taxonomy split by food family; kill the "Cooked Components" catch-all (mig 341, branch `claude/suspicious-knuth-b1b00c`).**
  ...
  - **Final orphans cleared (mig 342):** new `F-BEV-SOF` 💧 Water & Soft Drinks (ice, mineral water, soda, tonic) + `F-SUP-PRO` 💪 Protein & Supplements (whey, creatine, MCT); `SALE-PITA_POTATO*` → `KP-FIN-BRC` 🥖 Bread & Crackers. **Active Uncategorized is now 0** (was 19 before this pass). Applied to prod + verified.
