...
> Дополнение к `agent-rules.md`. Определяет как агенты открывают, закрывают сессии
...
Расширение таблицы из `agent-rules.md`:
...
- Этот протокол **дополняет** `agent-rules.md` (Tier 1/Tier 2 остаётся)
