...
3. **Admin 2-level editor** — ✅ **DONE (2026-06-03, redesigned for UX).** `/menu/modifiers` (ModifiersPage, NOT `/bom` — CEO-decided) is now a Loyverse-style **master-detail with two tabs** (no wall-of-rows). **By group** (`ModifierGroupsTab`): left = group list, right = the group's options (read-only, like Loyverse) + the **summary of attached dishes** with add/remove. DB-only writes via `dish_modifier_groups`. **Cost/margin (option→MOD food-cost) was REMOVED from this screen** per CEO — it confused the primary management flow; deferred to Phase 6 (owner-view). The `modifier_option_cost` table + Phase-1 backfill remain; only the UI is gone for now. Loyverse-mutating edits (add/remove option, price) stay in Phase 5.
