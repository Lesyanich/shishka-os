...
2. **Read path** — ✅ **DONE (2026-06-03).** `useModifierOptions(dishId)` rewritten onto the 2-level model: a dish's options = the options of its attached groups (`dish_modifier_groups` → `pos_loyverse_modifier_options`), left-joined to `modifier_option_cost` (cost/margin). Same `ModifierOption` shape (superset: + `group_name`, `cost`, `margin`), so the drawer chips (DishDrawer → CustomerTab → ModifierChips) keep working with no longer any read of `nomenclature_modifier_options`.
3. **Admin 2-level editor** — ✅ **DONE (2026-06-03, redesigned for UX).** `/menu/modifiers` (ModifiersPage, NOT `/bom` — CEO-decided) is now a Loyverse-style **master-detail with two tabs** (no wall-of-rows). **By group** (`ModifierGroupsTab`): left = group list; right = the group's options + the **summary of attached dishes** (searchable add / remove). Per option (`useModifierOptionEditing`): **editable selling price** staged to `modifier_option_overrides` (shows a "draft" badge when it differs from the live Loyverse price — applied by the Phase 5 push, because editing price in Loyverse detaches items) + **cost link** (option→MOD-* + portion → `modifier_option_cost`, immediate, ours) with computed cost + margin. So both price and cost are editable inside each group (CEO 2026-06-03). **Add/remove option** is also supported in-app (CEO 2026-06-03): `useModifierListOps` → edge `add_modifier_option`/`remove_modifier_option`, which mutate Loyverse and then **immediately `reattachAllDishes()`** (editing a modifier detaches its lists from items — verified the smoothies keep all 4 groups after an add+remove cycle). The cost editor shows a **live weight→cost/margin** preview as you type.
...
6. **Costing** — ✅ **DONE (2026-06-03).** OwnerTab (dish drawer) shows a "Modifier Options — cost & margin" table per dish: option · price · cost · margin (red when negative), fed by `useModifierOptions` (now carries cost/margin). Editing the ingredient links happens on `/menu/modifiers`.
7. **Deprecate** `nomenclature_modifier_options` — ⚠️ **PARTIAL (2026-06-03).** Frontend no longer reads it: the legacy binding UI (`AddBindingForm`, `BindingsTable`, `PulledMirrorSection`, `useModifierBindings`) is deleted and `useModifierOptions` moved to the 2-level model. **The table is NOT dropped** — `order_item_modifiers.modifier_option_id` is a FK into it (order history) and `fn_ingest_loyverse_receipt` still writes it on every Loyverse receipt (order→BOM pipeline). A physical drop requires: rewire `fn_ingest_loyverse_receipt` to resolve ordered options via `modifier_option_cost` (option→MOD+qty; note: no `slot` in the new model) + migrate/relax the `order_item_modifiers` FK + the `fn_refresh_loyverse_modifier_mirror` UPDATE block. Deferred as a separate, carefully-tested task (touches the live order/BOM pipeline).
